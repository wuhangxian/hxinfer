...
namespace hxinfer{
    ...
    private:
        ...
        // 中间激活值：7B 用 FP16（与 PyTorch 一致，避免反复 fp32↔fp16 精度损失）
        ...
        ModelConfig config_;
        DataType activation_dtype_;  // 激活值数据类型（7B=FP16, 15M=FP32）
    public:
        AttentionLayer(std::shared_ptr<Allocator> allocator, ModelConfig& config,
            ...
            // 根据权重类型决定激活值类型：权重 FP16 → 激活值 FP16
            ...
            curr_q_ = std::make_shared<Tensor>(allocator, curr_shapes, activation_dtype_);
            curr_k_ = std::make_shared<Tensor>(allocator, curr_shapes, activation_dtype_);
            curr_v_ = std::make_shared<Tensor>(allocator, curr_shapes, activation_dtype_);
            ...
            after_qktv_ = std::make_shared<Tensor>(allocator, curr_shapes, activation_dtype_);

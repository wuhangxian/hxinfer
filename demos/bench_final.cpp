...
int main(int argc, char** argv){
    ...
    const std::string TOKEN_IDS_FILE = "/tmp/sharegpt_tokens.txt";
    ...
    // Read pre-tokenized ShareGPT input
    ...
    while(pos < ids_str.size()){
        size_t next = ids_str.find(44, pos);
        if(next == std::string::npos) next = ids_str.size();
        tokens.push_back(std::stoi(ids_str.substr(pos, next-pos)));
        pos = next + 1;
    }
    tokens.insert(tokens.begin(), tokenizer.bos_id());
    tokens.resize(INPUT_LEN);

    std::cout << "\nInput: " << INPUT_LEN << " tokens (ShareGPT)" << std::endl;
    ...
    for(int w = 0; w < 5; w++){
        ...
        model->forward(input_single_gpu, logits_decode, pos_decode);

...
int main(){
    ...
    // Serial prefill using decode path
    ...
    for(int S : {256, 512, 1024}){
        ...
        std::cout << "Serial S=" << S << " next_token=" << next << " [" << tok.decode(next) << "]" << std::endl;
    }
    // Batch prefill
    for(int S : {256, 512, 1024}){
        if((int)all_tokens.size() < S) break;
        ...
        std::cout << "Batch  S=" << S << " next_token=" << next << " [" << tok.decode(next) << "]" << std::endl;
    }
    return 0;
}

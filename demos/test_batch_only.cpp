...
int main(){
    ...
    std::string prompt = "The quick brown fox jumps over the lazy dog. The quick brown fox jumps over the lazy dog. ";
    std::vector<int> all_tokens = tokenizer.encode(prompt);
    all_tokens.insert(all_tokens.begin(), tokenizer.bos_id());
    for(int S : {5, 10, 16, 32, 64, 128, 256, 512}){
        if((int)all_tokens.size() < S) {
            std::cout << "S=" << S << " not enough tokens (" << all_tokens.size() << ")" << std::endl;
            break;
        }

...
int main(){
    ...
    const std::string TOKEN_IDS_FILE = "/tmp/sharegpt_token_ids.txt";
    ...
    // Read pre-tokenized ShareGPT input (1024 token IDs)
    std::ifstream f(TOKEN_IDS_FILE);
    std::stringstream ss;
    ss << f.rdbuf();
    std::string ids_str = ss.str();
    std::vector<int> tokens;
    std::stringstream ids_ss(ids_str);
    std::string id_str;
    while(std::getline(ids_ss, id_str, ',')) {
        tokens.push_back(std::stoi(id_str));
    }
    tokens.insert(tokens.begin(), tokenizer.bos_id());
    int INPUT_LEN = std::min((int)tokens.size(), 1025);  // bos + 1024 = 1025
    tokens.resize(INPUT_LEN);

    std::cout << "\nInput: " << INPUT_LEN << " tokens (ShareGPT, pre-tokenized)" << std::endl;
    ...
    std::cout << "\n--- Output (first 500 chars) ---" << std::endl;
